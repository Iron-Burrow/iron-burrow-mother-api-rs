...
use crate::{
    adapters::price_indexer::{PriceIndexerClient, StrictLatestQuote, StrictPriceBatchError},
    application::balances::quote::{LatestPriceQuotes, PriceQuoteError, PriceQuoteResolution},
};

#[derive(Clone, Debug)]
pub struct PriceIndexerBalanceQuoteReader {
    client: PriceIndexerClient,
}

impl PriceIndexerBalanceQuoteReader {
    pub fn new(client: PriceIndexerClient) -> Self {
        Self { client }
    }
}

impl LatestPriceQuotes for PriceIndexerBalanceQuoteReader {
    async fn latest_quotes(
            ...
            .await
        {
            ...
            Err(error) => {
                ...
                Err(PriceQuoteError::from(error))
            }
        }
    }
}

impl From<StrictPriceBatchError> for PriceQuoteError {
...
mod tests {
    ...
    fn maps_strict_client_errors_to_balance_quote_classes() {
        for error in [
            ...
            assert_eq!(
                PriceQuoteError::from(error),
                PriceQuoteError::ProviderUnavailable
            );
        }
        ...
        for error in [
            ...
            assert_eq!(PriceQuoteError::from(error), PriceQuoteError::InternalError);
        }
    }
}
