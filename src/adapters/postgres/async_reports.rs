...
impl From<AsyncReportRow> for AsyncReport {
    fn from(row: AsyncReportRow) -> Self {
        Self {
...
impl AsyncReportRepository {
    ...
    pub(crate) async fn find_owned(
        ...
        account_id: Uuid,
        public_id: &str,
    ) -> Result<Option<AsyncReport>, RepositoryError> {
    ...
    pub(crate) async fn mark_running(&self, public_id: &str) -> Result<(), RepositoryError> {
        sqlx::query("update mother_api.async_report set status='running', started_at=coalesce(started_at,now()) where public_id=$1 and status='accepted'").bind(public_id).execute(&self.pool).await.map_err(RepositoryError::new).map(|_| ())
    }
    pub(crate) async fn complete(
        ...
        sqlx::query("update mother_api.async_report set status='completed', report=$2, report_digest=$3, completed_at=now() where public_id=$1 and status in ('accepted','running')").bind(public_id).bind(report).bind(digest).execute(&self.pool).await.map_err(RepositoryError::new).map(|result| result.rows_affected() == 1)
    }
    pub(crate) async fn fail(&self, public_id: &str) -> Result<bool, RepositoryError> {
        sqlx::query("update mother_api.async_report set status='failed', failure_code='execution_failed', failed_at=now() where public_id=$1 and status in ('accepted','running')").bind(public_id).execute(&self.pool).await.map_err(RepositoryError::new).map(|result| result.rows_affected() == 1)
    }
}
